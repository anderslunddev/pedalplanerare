 ...
 * number).
 */
public record PedalResponse(UUID id, String name, double width, double height, String color, double x, double y,
		int placement) {
	public static PedalResponse from(Pedal pedal) {
		return new PedalResponse(pedal.id().value(), pedal.name().value(), pedal.surfaceArea().width(),
